...
# 交易人数分布结果磁盘缓存（服务重启后仍有效）
TRADING_DISTRIBUTION_CACHE_TTL_SECONDS = 6 * 60 * 60  # 6小时；如需更久可调大
_cache_lock = threading.Lock()

def _cache_dir() -> str:
    base = os.path.dirname(os.path.abspath(__file__))
    d = os.path.join(base, ".cache_trading_distribution")
...
def _cache_load_df(prefix: str, payload: dict) -> Optional[pd.DataFrame]:
    key = _cache_key(prefix, payload)
    pkl_path, meta_path = _cache_paths(key)
    try:
...
def _cache_save_df(prefix: str, payload: dict, df: pd.DataFrame, ttl_seconds: int = TRADING_DISTRIBUTION_CACHE_TTL_SECONDS) -> None:
    key = _cache_key(prefix, payload)
    pkl_path, meta_path = _cache_paths(key)
    try:
...
def get_valid_login_codes() -> set:
    try:
        # 仅用于“排除测试组”的口径：只从 js_mt4_account 取非测试组账号，
        # 不额外要求“必须在 v_mt4_trades_filtered 出现过交易记录”。
        #
        # 注意：你要求的 “v_mt4_trades_filtered 仅取 2023-10-31 23:59:59 及之前、
        # 且 LOGIN 范围 + cmd 条件” 应该只应用在查询 v_mt4_trades_filtered 的 SQL 上，
        # 而不是影响其它表/其它口径的账号集合。
...
def get_account_reg_year_df() -> pd.DataFrame:
    """
    从 js_mt4_account 中提取正常账户的开户年份（reg_time），并映射到 login_code。
    仅保留 login_code 为 6 位且不为 '0' 的记录。
    """
    try:
        sql = """
            SELECT id, reg_time, `group`
            FROM js_mt4_account
            WHERE `group` NOT IN ('99', 'G99', 'manager')
        """
        df = pd.read_sql(sql, engine_finance)
        if df.empty:
        ...
        # 去重：同一 login_code 只保留一条记录（优先最早年份）
...
def query_trading_distribution_df(
    ...
    """
    交易人数分布：
    行：交易年度（start_year～end_year）
    列：年度、交易人数（所选交易人数类型）、开户年份 2015～2026（对应 js_mt4_account.reg_time 的年份）

    关键优化：
    - 在 SQL 中先按 “年 × login” 聚合（GROUP BY），减少明细量；
    - 对 v_mt4_trades_filtered 按时间片分段查询，避免大范围扫描；
    - 使用 lru_cache 缓存结果。
    """
    try:
        ...
        if pre_start <= pre_end:
            ...
            with engine_trade_mt4.connect() as conn:
                while cur_start <= pre_end:
                    ...
                    try:
                        df_open_pre = pd.read_sql(
                            sql_open_pre,
                            conn,
                            params={"start_date": cur_start_str, "end_date": cur_end_str},
                        )
                        df_close_pre = pd.read_sql(
                            sql_close_pre,
                            conn,
                            params={"start_date": cur_start_str, "end_date": cur_end_str},
                        )
                    except Exception as e:
                        print(f"query_trading_distribution_df v_mt4 range chunk failed {cur_start_str}~{cur_end_str}: {e}")
                        df_open_pre = pd.DataFrame()
                        df_close_pre = pd.DataFrame()
        ...
        # ---------- 2) 新系统数据：js_day_jyr（SQL 内聚合，一次查完区间） ----------
        ...
        if post_start <= post_end:
            ...
            sql_open_post = text(
                """
                SELECT YEAR(nearmonth1) AS trade_year, login AS login
                FROM js_day_jyr
                WHERE nearmonth1 >= :start_date AND nearmonth1 <= :end_date
                GROUP BY trade_year, login
                """
            ...
            sql_close_post = text(
                """
                SELECT YEAR(nearmonth) AS trade_year, login AS login
                FROM js_day_jyr
                WHERE nearmonth >= :start_date AND nearmonth <= :end_date
                GROUP BY trade_year, login
                """
            ...
            with engine_trade.connect() as conn:
                df_open_post = pd.read_sql(
                    sql_open_post,
                    conn,
                    params={"start_date": post_start_str, "end_date": post_end_str},
                )
                df_close_post = pd.read_sql(
                    sql_close_post,
                    conn,
                    params={"start_date": post_start_str, "end_date": post_end_str},
                )
        ...
        df_all = df_all.dropna(subset=["trade_year", "login", "type"])

        # 仅使用有效 login_code
        if valid_codes and not df_all.empty:
            df_all["login_code"] = extract_login_code(df_all["login"])
            df_all = df_all[df_all["login_code"].isin(valid_codes)]

        # 初始化每年集合
        year_type_users: dict[int, dict[str, set]] = {y: {lbl: set() for lbl in type_labels} for y in years}
        ...
        if not df_all.empty:
            for y, grp in df_all.groupby("trade_year"):
                try:
                    ...
                    continue
                if y_int not in year_type_users:
                    continue
        ...
        # ---------- 3. 组装结果表：只保留当前选择的交易人数类型 ----------
        rows = []
        for year in years:
            type_map = year_type_users.get(year, {lbl: set() for lbl in type_labels})
            users = type_map.get(user_type, set()) or set()
            total_cnt = len(users)
            ...
            if not users or acct_df.empty:
                ...
                for ry in reg_years:
                    row[str(ry)] = int(vc.get(ry, 0))

            rows.append(row)

        if not rows:
            return pd.DataFrame(
                columns=["年度", "交易人数类型"] + [str(y) for y in reg_years]
            )
        ...
        print(f"Query trading distribution failed: {e}")
        import traceback
        traceback.print_exc()
        return pd.DataFrame(
            columns=["年度", "交易人数类型"] + [str(y) for y in range(2015, 2026 + 1)]
        )
...
# ========== 接口四：交易人数分布（HTML） ==========
@app.get(
    "/pyapi/user/distribution",
    response_class=HTMLResponse,
    summary="交易人数分布：交易年度 × 开户年度（2015-2026）",
)
def trading_distribution_html(
    start_year: int = Query(2015, ge=2000, le=2035, description="起始交易年度"),
    end_year: int = Query(2026, ge=2000, le=2035, description="结束交易年度"),
    user_type: str = Query("开仓人数", description="交易人数类型"),
):
    # 并发保护：同一时间只允许少量请求跑重查询，避免数据库被打爆
    if not trading_distribution_semaphore.acquire(blocking=False):
        raise HTTPException(status_code=429, detail="交易人数分布计算中，请稍后重试（已限流保护数据库）")
    ...
    cache_payload = {
        ...
        "user_type": user_type,
        "logic": "trade_dist_v3_mt4_login_cmd_filters",
    }

    try:
        # 全局（磁盘）缓存：服务重启后仍可复用，显著减少扫库次数
        with _cache_lock:
            df = _cache_load_df("trading_distribution", cache_payload)

        if df is None or df.empty:
            df = query_trading_distribution_df(start_year, end_year, user_type=user_type)
            # 防御性过滤
            df = df[(df["年度"] >= start_year) & (df["年度"] <= end_year)]
            with _cache_lock:
                _cache_save_df("trading_distribution", cache_payload, df)
        else:
            df = df[(df["年度"] >= start_year) & (df["年度"] <= end_year)]
    finally:
    ...
    if df.empty:
        return """<!DOCTYPE html><html><head><meta charset="UTF-8"><title>交易人数分布</title></head>
<body style="font-family:system-ui;padding:20px;"><h2>交易人数分布</h2><p>暂无数据</p></body></html>"""

    df = df.rename(columns={"交易人数类型": "交易人数"})
    df = df.rename(columns={"交易人数": "交易人数类型"})
    ...
    <head>
        ...
        <style>
            ...
            .container {{ max-width:1200px; background:white; padding:24px; border-radius:10px; box-shadow:0 4px 16px rgba(0,0,0,0.08); overflow-x:auto; }}
            ...
            .controls {{ display:flex; flex-wrap:wrap; justify-content:center; align-items:center; gap:0.75rem; margin-bottom:1rem; font-size:0.9rem; }}
            .controls select {{ padding:4px 8px; font-size:0.9rem; }}
            .controls input[type="number"] {{ width:80px; padding:4px 6px; font-size:0.9rem; }}
            .controls button {{ padding:4px 10px; font-size:0.9rem; cursor:pointer; }}
            .stat-table {{ width:100%; border-collapse:collapse; font-size:14px; }}
            .stat-table th {{ background:#0066cc; color:white; padding:10px; text-align:center; white-space:nowrap; }}
    ...
    <body>
        <div class="container">
            <h2>交易人数分布</h2>
            <div class="controls">
                <form method="get" action="/pyapi/user/distribution">
                    <label>交易人数类型：
                        <select name="user_type">
                            <option value="开仓人数"{" selected" if user_type == "开仓人数" else ""}>开仓人数</option>
                            <option value="平仓人数"{" selected" if user_type == "平仓人数" else ""}>平仓人数</option>
                            <option value="开仓+平仓 人数"{" selected" if user_type == "开仓+平仓 人数" else ""}>开仓+平仓 人数</option>
                            <option value="开仓人数(A)"{" selected" if user_type == "开仓人数(A)" else ""}>开仓人数(A)</option>
                            <option value="平仓人数(A)"{" selected" if user_type == "平仓人数(A)" else ""}>平仓人数(A)</option>
                            <option value="开仓+平仓 人数(A)"{" selected" if user_type == "开仓+平仓 人数(A)" else ""}>开仓+平仓 人数(A)</option>
                        </select>
                    </label>
                    <label>起始年度：
                        <input type="number" name="start_year" value="{start_year}" min="2000" max="2035" />
                    </label>
                    <label>结束年度：
                        <input type="number" name="end_year" value="{end_year}" min="2000" max="2035" />
                    </label>
                    <button type="submit">查询</button>
                </form>
            </div>
            <p class="filter">
                行：年度（{start_year}～{end_year}），第二列为所选“{user_type}”对应的去重人数；<br>
                列：开户年份（2015～2026），表示这些人数按开户年份（reg_time 年份）拆分后的分布。
            </p>
            {table_html}
            <p class="note">
                交易人数类型列中的数字为该年度下、所选交易行为类型对应的去重人数。
            </p>
        </div>
    </body>
    </html>
